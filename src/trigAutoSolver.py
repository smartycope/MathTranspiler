...
class TriangleSolver(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        todo('add little checkboxes next to all the inputs and have them lock/keep the value during reset')
        #// todo('fails when someLine = 200, theta = 60 (but not when 40), and fullBase = 100')
        todo('add a reset as well as a resetAll button')
    ...
    def __setitem__(self, key, val):
        ...
        if self.values[key] is not None:
            if self.isDeg.isChecked() and key in self.angles:
                ...
                val = deg(val)
            val = val.simplify() if self.solveSymbolic.isChecked() else self.round(val.evalf(), self.roundTo.value())
            # debug(self[key], key)
    ...
    def calculate(self):
        ...
        def calc(setTo, expr):
            ...
            if self[setTo] is None:
                ...
                # Double check to make sure that we haven't messed up our calculations
                try:
                    # ... *mumble mumble* rounding errors *mumble mumble* ...
                    if self[setTo] != expr or not closeEnough(self[setTo].evalf(), expr.evalf(), 1):
                        warning(f"{setTo} ({self[setTo]}) != {expr}")
        ...
        #* If we have fullAngle and a line adjacent to it, we can make an imaginary triangle on the outside,
        ...
        if have('fullAngle'):
            for hyp, base in imaginaryOutsideTriangles:
                if have(hyp):
                    theta = rad(180) - self['fullAngle']
                    debug(theta)
                    debug(self[hyp])
                    debug(cos(theta))
                    debug(sin(theta))
                    calc(base,     self[hyp] * cos(theta))
                    calc('height', self[hyp] * sin(theta))
...
class TriangleSolver(QDialog):
    lines  = ('yoMama', 'leftBase', 'rightBase', 'fullBase', 'height', 'someLine')
    angles = ('leftAngle', 'rightAngle', 'fullAngle', 'theta', 'notTheta')
    def __init__(self, parent=None):
        super().__init__(parent=parent)
        uic.loadUi(join(ROOT, "ui/trigInput.ui"), self)
